import { registry, groupBy, register } from "@ark/util";
...
if (suffix !== 2) {
    ...
    if (paths.length > 1) {
        console.warn(`Registries were initialized at the following paths:` +
            paths
                .map(path => `	${path} (@ark/util version ${byPath[path][0].version})`)
                .join("\n"));
    }
}
