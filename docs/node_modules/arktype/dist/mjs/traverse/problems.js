...
class ProblemArray extends Array {
    ...
    add(code, source, opts) {
        ...
        const path = Path.from(opts?.path ?? _classPrivateFieldGet(this, _state).path);
...
export const Problems = ProblemArray;
const capitalize = (s)=>s[0].toUpperCase() + s.slice(1);
export const domainsToDescriptions = (domains)=>domains.map((objectKind)=>domainDescriptions[objectKind]);
export const objectKindsToDescriptions = (kinds)=>kinds.map((objectKind)=>objectKindDescriptions[objectKind]);
export const describeBranches = (descriptions)=>{
...
export const problemCodes = objectKeysOf(defaultProblemConfig);
...
export const compileProblemWriters = (input)=>{
    if (!input) {
